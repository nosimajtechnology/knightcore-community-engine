...
When an idea is already present, select the smallest fitting mode and proceed. Ask at most one question, and only when the answer materially changes the result.
...
- any grounded image, PS2 scene, storyboard, or fidelity repair: [rendering-grounding.md](references/rendering-grounding.md) and [bloom-and-glow.md](references/bloom-and-glow.md)
- mode selection, approval stages, storyboards, or episodes: [modes.md](references/modes.md)
...
5. verified Knightcore official or community-linked material
6. established community convention
7. inspected original-platform PS2 references for rendering only
...
## Ground every PS2 output before generation

For every grounded still, meme image, scene, cinematic, Tape, commercial, or episode board, follow [rendering-grounding.md](references/rendering-grounding.md):
...
When image generation is available and the user requests an image, first frame, or storyboard, generate it after grounding. When unavailable or the user requests `prompt only`, provide a complete copy-paste prompt and never imply it was rendered.

If a generated frame fails one major archetype/construction/bloom rule or two other fidelity rules, do not present it as passing. Lock the correct layers, attempt one narrow repair when isolated, and regenerate when the entire asset/light/camera stack is modern. If a second attempt still fails, report the limitation briefly and ask whether to try one more grounded generation.

## Route and execute

Use [modes.md](references/modes.md) to choose the smallest mode that fits. STILL remains first-class; do not force a storyboard or video workflow onto a one-image request. Honor explicit constraints such as `one image only`, `no video`, `prompt only`, `Seedance`, `Kling`, `under 3500 characters`, `no music`, and `use this approved frame`.
...
Treat `approved`, `lock it`, `perfect`, and clear equivalents as approval. After first-frame approval, lock archetype/variant, wardrobe, weapons, props, creatures, environment, geography, positions, light, palette, weather, bloom family, screenshot set, rendering contract, and current action state.
...
Create a model-neutral motion brief before applying [model-adapters.md](references/model-adapters.md). Deliver:
...
Preserve approved archetype/continuity, action progression, PS2 construction and bloom, motion weight, decisive negatives, then atmosphere. Do not invent provider limits or redesign the knight during animation.
